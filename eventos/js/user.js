/**
 * @file este modulo se encarga de todas las operaciones el tablero de eventos
 */
...
controller('cardsCtrl', ['$scope', '$http', '$rootScope', function($scope, $http, $rootScope) {
        ...
        $scope.verMapa = function() {
            // mostramos el mapa y cargamos los datos de ubicacion
            $scope.mapa = true;
            $http({
                method: 'POST',
                ...
                $scope.map = new google.maps.Map(document.getElementById('mapa'), {
                    center: {
                        lat: $scope.lugar.LATITUD,
                        lng: $scope.lugar.LONGITUD
                    },
                    zoom: 14
                });
        ...
        $scope.selsala = function() {
            // al seleccionar la sala obtenemos los datos de la misma 
        ...
        // cuando el usuario requiere boletos se ajusta la disponibilidad y
        // se calcula el monto
        ...
        $scope.reset = function() {
            // accionado cuando se cancela una compra
            $scope.cadulto = 0;
            $scope.cnino = 0;
            $scope.cvejez = 0;
            $scope.cestudiante = 0;
        ...
        // obtener todso los eventos disponibles
        $http({
            method: 'GET',
    ...
    .controller('buscadorCtrl', ['$scope', '$http', '$rootScope', function($scope, $http, $rootScope) {
        ...
        $scope.buscar = function() {
            ...
            $http({
                ...
                // actualiza los eventos mostrados en el tablero
                $rootScope.eventos = response.data;
    ...
    .directive('tabCard', [function() {
        // crea una nueva carta en el tablero
        return {
            restrict: 'A',
...
$(document).ready(function() {
    // permite el autocompletado en el buscador
